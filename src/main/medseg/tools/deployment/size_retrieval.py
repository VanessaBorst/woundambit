...
def calculate_wound_dimensions(wound_mask, pixel_to_mm_ratio, image=None, multi_wound_mode="largest"):
    ...
    return wound_area_mm2, wound_width_mm, wound_height_mm, annotated_image
...
def process_image(image_path, mask_paths, output_dir, multi_wound_mode):
    ...
    for mask_path in mask_paths:
        ...
        wound_area_mm2, wound_width_mm, wound_height_mm, annotated_image = (
            calculate_wound_dimensions(wound_mask, pixel_to_mm_ratio, image, multi_wound_mode)
        )

        annotated_name = os.path.basename(mask_path).replace(".png", "_annotated.jpg")
        ...
        results.append((os.path.basename(image_path).split(".")[0], os.path.basename(mask_path).split("_")[2],
                        wound_area_mm2, wound_width_mm, wound_height_mm))
...
def main(data_dir, output_csv, output_dir, multi_wound_mode):
    ...
    # Identify unique image filenames (e.g., "Image_01.png", "Image_02.png")
    image_files = sorted({f for f in os.listdir(data_dir) if f.startswith("Image_")
                          and f.endswith(".jpg") and "_mask" not in f})
    ...
    for image_file in image_files:
        ...
        # Find all corresponding mask files (e.g., "Image_01_modelX_mask.png")
    ...
    df = pd.DataFrame(data, columns=["image_id", "model_name", "area_mm2", "width_mm", "height_mm"])
