...
class _BaseWrapper():
    ...
    def _isolate_class(self, output, label):
        ...
        if label is None:
            result = torch.where(output > 0.5, torch.ones_like(output), torch.zeros_like(output))
    ...
    def _extract_metadata(self, input,
                          output):  # TODO: Does not work for classification output (shape: (1, 1000)), merge with the one in medcam_inject
    ...
    @staticmethod
    def _normalize_per_channel(attention_map):
