...
from medseg.evaluation.medcam.guided_backpropagation import GuidedBackPropagation
from medseg.evaluation.medcam.grad_cam import GradCAM
from medseg.evaluation.medcam.guided_grad_cam import GuidedGradCam
from medseg.evaluation.medcam.grad_cam_pp import GradCamPP
...
def inject(model, output_dir=None, backend='gcam', layer='auto', label=None, data_shape='default',
           save_maps=False, save_pickle=False, save_scores=False, evaluate=False, metric='wioa', threshold='otsu',
           retain_graph=False,
    ...
    if "TransNeXt" in type(model).__name__:
        # Deep copy does not work since some modules are not pickable
        # Instead of cloning the whole model with deepcopy(), save and load its weights using state_dict()
        model_clone = type(model)()  # Create a new instance of the model class
        model_clone.load_state_dict(model.state_dict())
        model_clone.to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
    else:
    ...
    model_backend, heatmap = _assign_backend(backend, model_clone, layer, None,
...
def save_attention_map(self, attention_map, raw_input=None):
    ...
    medcam_utils.save_attention_map(
        filename=self.medcam_dict['output_dir'] + "/" + self.medcam_dict['current_layer'] + "/attention_map_" +
...
def forward(self, batch, label=None, mask=None, img_names=None, raw_input=None, is_aux_test_data=False,
            xai_predictions_hook=None, dataset_name=None):
    """
    ...
    if self.medcam_dict['enabled']:
        with torch.enable_grad():
            ...
            if self.medcam_dict['layer'] == 'full' and not self.medcam_dict['tested']:
                raise ValueError(
                    "Layer mode 'full' requires a test run either during injection or by calling test_run() afterwards")
            output, attention_map, batch_size, channels, data_shape = self.medcam_dict[
                'model_backend'].generate_attention_map(batch, label)
            if attention_map:
                xai_predictions_hook(dataset_name, img_names,
                                     attention_map) if xai_predictions_hook is not None else None
                ...
                scores = self._process_attention_maps(attention_map, mask, batch_size, channels, raw_input, img_names,
                                                      is_aux_test_data)
                output = self._replace_or_return(output, attention_map, batch.shape[2:])
            else:  # If no attention maps could be extracted
                xai_predictions_hook(dataset_name, img_names, None) if xai_predictions_hook is not None else None
    ...
    if batch is not None and not self.medcam_dict['tested']:
        ...
        if not internal:
            ...
            if self.medcam_dict['output_dir'] is not None:
                np.savetxt(self.medcam_dict['output_dir'] + '/registered_layers.txt',
                           np.asarray(registered_hooks).astype(str), fmt="%s")
...
def _assign_backend(backend, model, target_layers, postprocessor, retain_graph):
    ...
    if backend == "gbp":
        ...
        return GradCAM(model=model, target_layers=target_layers, postprocessor=postprocessor,
                       retain_graph=retain_graph), True
    elif backend == "ggcam":
        return GuidedGradCam(model=model, target_layers=target_layers, postprocessor=postprocessor,
                             retain_graph=retain_graph), False
    elif backend == "gcampp":
        return GradCamPP(model=model, target_layers=target_layers, postprocessor=postprocessor,
                         retain_graph=retain_graph), True
    else:
        raise ValueError("Backend does not exist")


def _process_attention_maps(self, attention_map, mask, batch_size, channels, raw_input, img_names, is_aux_test_data):
    ...
    for layer_name in attention_map.keys():
        ...
        for j in range(batch_size):
            if not is_aux_test_data:
                self._save_attention_map(attention_map[layer_name][j], layer_output_dir, j, raw_input[j],
                                         filename=img_names[j])
            for k in range(channels):
                ...
                if self.medcam_dict['evaluate']:
                    ...
                    score = self.medcam_dict['Evaluator'].comp_score(attention_map_single, mask[j][k].squeeze(),
                                                                     layer=layer_name, class_label=k)
                    batch_scores[layer_name].append(score)
    return batch_scores


def _save_attention_map(self, attention_map, layer_output_dir, j, raw_input, filename):
    ...
    if self.medcam_dict['save_maps']:
        medcam_utils.save_attention_map(filename=f"{layer_output_dir}/XAI_{filename}", attention_map=attention_map,
                                        heatmap=self.medcam_dict['heatmap'], raw_input=raw_input)
...
def _replace_or_return(self, output, attention_map, data_shape):
    ...
    if self.medcam_dict['_replace_output'] or self.medcam_dict['return_attention']:
        if len(attention_map.keys()) == 1:
            output_attention_map = torch.FloatTensor(self.medcam_dict['current_attention_map']).to(
            ...
            if data_shape is not None:  # If data_shape is None then the task is classification -> return unchanged attention map
                ...
                print(f"Interpolating size to {data_shape}")
                # output_attention_map = output_attention_map
            if self.medcam_dict['_replace_output']:
            ...
            raise ValueError(
                "Not possible to replace output or return attention map when layer is set to 'full', only with 'auto' or a manually set layer")
    return output
