...
def save_attention_map(filename, attention_map, heatmap, raw_input):
    """
    Saves an attention map overlayed on the input image.
    Args:
        filename: The save path, including the name, excluding the file extension.
        attention_map: The attention map in DxHxW format or HxW format.
        heatmap: If the attention map should be saved as a heatmap. True for gcam and gcampp. False for gbp and ggcam.
        raw_input: The raw input image tensor.
    """
    # Convert raw input to numpy and normalize
    image_np = raw_input.cpu().numpy().transpose(1, 2, 0)
    image_np = (image_np - image_np.min()) / (image_np.max() - image_np.min())

    # Handle attention map with depth (D, H, W) or just (H, W)
    if attention_map.ndim == 3:
        # Select the first channel if multiple channels are present
        attention_map_np = attention_map[0]  # Shape is now (H, W)
    else:
        attention_map_np = attention_map

    # Convert to numpy if it's a tensor
    if isinstance(attention_map_np, torch.Tensor):
        attention_map_np = attention_map_np.cpu().numpy()

    # Resize attention map to match the image size using cv2
    attention_map_resized = cv2.resize(attention_map_np, (image_np.shape[1], image_np.shape[0]))

    # Normalize resized attention map
    epsilon = 1e-8
    attention_map_resized = (attention_map_resized - attention_map_resized.min()) / (
            attention_map_resized.max() - attention_map_resized.min() + epsilon)

    # The following is needed to ensure that dimensions of the output file are maintained!
    cmap = plt.get_cmap('jet')
    attention_map_colored = cmap(attention_map_resized)
    # Colormap generates a RGBA color for each array element -> omit alpha channel
    attention_map_colored = attention_map_colored[..., :3]

    if heatmap:
        overlay = 0.5 * image_np + 0.5 * attention_map_colored
    else:
        overlay = attention_map_colored

    # Not needed
    # overlay = np.clip(overlay, 0, 1)
    # overlay_uint8 = (overlay * 255).astype(np.uint8)
    # plt.imsave(filename, overlay_uint8)

    plt.imsave(filename, overlay)
    plt.close()

    # # OLD VERSION: Overlay and save the image
    # plt.imshow(image_np)
    # plt.imshow(attention_map_resized, cmap='jet', alpha=0.5 if heatmap else 1.0)
    # plt.axis('off')
