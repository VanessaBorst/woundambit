...
# https://github.com/DaiShiResearch/TransNeXt/blob/main/segmentation/upernet/transnext_cuda.py
# Renamed model variants from transnext_<size> -> TransNeXt<Size>
# Removed unused num_classes argument for TransNeXt
# Removed @BACKBONE annotations
# Added in_chans to variant constructors, changed order (kwargs as the last three params)
# Changed init_weights methods (changed pretrain param to pretrained_filename
# in the arguments and adapted corresponding weight loading functionality instead of mmcv.runner load_checkpoint)
...
class TransNeXt(nn.Module):
    ...
    def __init__(self, img_size=224, pretrain_size=None, window_size=[3, 3, 3, None],
                 patch_size=16, in_chans=3, embed_dims=[64, 128, 256, 512],
                 num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=False, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], num_stages=4, pretrained_filename=None, is_extrapolation=False):
        super().__init__()
        ...
        if pretrained_filename is not None:
            self.init_weights(pretrained_filename)
    ...
    def init_weights(self, pretrained_filename):
        if isinstance(pretrained_filename, str):
            ...
            for k in list(checkpoint.keys()):
                if k not in self.state_dict().keys():
                    del checkpoint[k]
            # There should be no missing keys, so strict=False is not required
            self.load_state_dict(checkpoint)
        else:
            raise ValueError(f'Pretrained filename {pretrained_filename} needs to be a string')
...
class TransNeXtTiny(TransNeXt):
    def __init__(self, **kwargs):
        super().__init__(window_size=[3, 3, 3, None],
                         ...
                         pretrained_filename="transnext_tiny_224_1k.pth", pretrain_size=224,
...
class TransNeXtSmall(TransNeXt):
    def __init__(self, **kwargs):
        super().__init__(window_size=[3, 3, 3, None],
                         patch_size=4, embed_dims=[72, 144, 288, 576], num_heads=[3, 6, 12, 24],
                         mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
                         ...
                         pretrained_filename="transnext_small_224_1k.pth", pretrain_size=224,
                         img_size=kwargs['img_size'],
                         in_chans=kwargs['in_chans'],
                         is_extrapolation=kwargs['is_extrapolation'])
...
class TransNeXtBase(TransNeXt):
    def __init__(self, **kwargs):
        super().__init__(window_size=[3, 3, 3, None],
                         ...
                         drop_rate=0.0, drop_path_rate=0.7,
                         pretrained_filename="transnext_base_224_1k.pth", pretrain_size=224,
                         img_size=kwargs['img_size'],
                         in_chans=kwargs['in_chans'],
                         is_extrapolation=kwargs['is_extrapolation'])