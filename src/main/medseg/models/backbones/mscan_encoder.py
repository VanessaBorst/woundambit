...
# Changed:
# Changed BaseModule -> nn.Module in all classes
# MSCAN: norm_cfg defaults to None and is dynamically chosen based on settings (multi-gpu or not)
# MSCAN: Removed init_cfg=None argument
# MSCAN: Changed int_weight logic to custom pre-trained loading
...
class MSCAN(nn.Module):
    def __init__(self,
                 ...
                 depths=[3, 4, 6, 3],
                 num_stages=4,
                 norm_cfg=None, #dict(type='SyncBN', requires_grad=True),
                 pretrained_filename: Optional[str] = None):
        super(MSCAN, self).__init__()

        # Dynamically set norm_cfg if not provided (default is SyncBN in the original implementation)
        if norm_cfg is None:
            norm_type = 'SyncBN' if torch.cuda.device_count() > 1 else 'BN'
