...
from medseg.models.common.mmpretrain_utils import GRN, build_norm_layer
from medseg.util.path_builder import PathBuilder


# Copied and adpated from MMPretrain (https://github.com/open-mmlab/mmpretrain/blob/main/mmpretrain/models/backbones/convnext.py)
# Changed:
# Use MMEngine MODELS instead of MMPretrain version
# ConvNext: BaseBackbone -> BaseModule (in MMPretrain, BaseBackbone inherits from this without deeper own functionalites)
# ConvNext: added pretrained_filename as argument and weight loading logic
        ...
        if isinstance(pretrained_filename, str):
            ...
            for k in list(checkpoint.keys()):
                if k not in self.state_dict().keys():
                    # Should be only 2 keys: weight and bias of head
                    del checkpoint[k]
            # # 8 missing (norm 0 -3, weight and bias)
            # counter = 0
